...
async function loadBookDetails(bookId) {
    try {
        const res = await fetch(`https://sdkapi.hhlqilongzhu.cn/api/fanqie_tingshu/?key=Dragon9430124081FEABFA626884BD28AF1BDF&book_id=${bookId}`);
        const data = await res.json();
        if (data.code === 200 && data.data) {
...
async function playChapter(index) {
    ...
    try {
        const res = await fetch(`https://sdkapi.hhlqilongzhu.cn/api/fanqie_tingshu/?key=Dragon9430124081FEABFA626884BD28AF1BDF&video_id=${chapter.video_id}`);
        const data = await res.json();
