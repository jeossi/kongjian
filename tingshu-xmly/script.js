...
async function performSearch() {
    ...
    try {
        const res = await fetch(`https://sdkapi.hhlqilongzhu.cn/api/ximalaya/?key=Dragon9430124081FEABFA626884BD28AF1BDF&name=${encodeURIComponent(query)}`);
...
function renderSearchResults(books) {
    ...
    books.forEach(b => {
        ...
        const isFavorited = isBookFavorited(b.albumId);
        card.innerHTML = `
            <img src="${b.cover}" alt="${b.title}" class="book-cover">
            <div class="book-info">
                <div class="book-title">${b.title}</div>
                <div class="book-author">播音: ${b.Nickname}</div>
                ...
                <div class="book-buttons">
                    <button class="book-btn favorite-book-btn ${isFavorited ? 'favorited' : ''}" 
                        data-bookid="${b.albumId}" title="${isFavorited ? '已收藏' : '收藏'}">
                        <i class="${isFavorited ? 'fas' : 'far'} fa-star"></i> 收藏
                    </button>
                    <button class="book-btn share-book-btn" data-bookid="${b.albumId}" title="分享">
                        <i class="fas fa-share-alt"></i> 分享
                    </button>
                    <button class="book-btn play-book-btn" data-bookid="${b.albumId}" title="播放">
...
// ================= 书籍功能 =================
async function loadBookDetails(albumId) {
    try {
        const res = await fetch(`https://sdkapi.hhlqilongzhu.cn/api/ximalaya/?key=Dragon9430124081FEABFA626884BD28AF1BDF&albumId=${albumId}`);
        const data = await res.json();
        if (data.data && Array.isArray(data.data)) {
            // 找到对应的书籍信息
            const bookInfo = state.searchResults.find(book => book.albumId == albumId);
            state.currentBook = {
                albumId: albumId,
                book_name: data.albumTitle || (bookInfo ? bookInfo.title : '未知书籍'),
                author: bookInfo ? bookInfo.Nickname : '未知作者',
                category: bookInfo ? bookInfo.type : '未知分类',
                book_pic: bookInfo ? bookInfo.cover : '',
                trackTotalCount: data.trackTotalCount || data.data.length
            };
            ...
            if (state.chapters.length) {
                const savedChapterIndex = getSavedChapterIndex(albumId);
...
async function playChapter(index) {
    ...
    try {
        const res = await fetch(`https://sdkapi.hhlqilongzhu.cn/api/ximalaya/?key=Dragon9430124081FEABFA626884BD28AF1BDF&trackId=${chapter.trackId}`);
        const data = await res.json();
        if (data.code === 200 && data.url) {
            const audioUrl = proxyUrl(data.url);
...
function toggleFavorite(e) {
    ...
    const bookIndex = favorites.findIndex(f => f.albumId === bookId);
    if (bookIndex !== -1) {
        favorites.splice(bookIndex, 1);
        btn.innerHTML = '<i class="far fa-star"></i> 收藏';
        btn.classList.remove('favorited');
    } else {
        const book = state.searchResults.find(b => b.albumId == bookId);
        if (book) {
            favorites.push({
                albumId: book.albumId,
                title: book.title,
                cover: book.cover,
                author: book.Nickname
            });
...
function isBookFavorited(albumId) {
    const storedFavorites = localStorage.getItem('bookFavorites');
    if (!storedFavorites) return false;
    return JSON.parse(storedFavorites).some(f => f.albumId == albumId);
}
...
function renderFavorites() {
    ...
    favorites.forEach(book => {
        ...
        const albumId = book.albumId || book.book_id; // 兼容旧数据
        item.innerHTML = `
            ...
            <button class="remove-favorite" data-bookid="${albumId}">
                <i class="fas fa-times"></i>
            </button>`;
        item.addEventListener('click', () => loadBookDetails(albumId));
...
function removeFavorite(albumId) {
    const storedFavorites = localStorage.getItem('bookFavorites');
    let favorites = storedFavorites ? JSON.parse(storedFavorites) : [];
    favorites = favorites.filter(f => f.albumId != albumId && f.book_id != albumId); // 兼容旧数据
    localStorage.setItem('bookFavorites', JSON.stringify(favorites));
    renderFavorites();
    document.querySelectorAll(`.favorite-book-btn[data-bookid="${albumId}"]`).forEach(btn => {
        btn.innerHTML = '<i class="far fa-star"></i> 收藏';
        btn.classList.remove('favorited');
    });
}
...
function shareBook(e) {
    e.stopPropagation();
    const albumId = e.currentTarget.dataset.bookid;
    const book = state.searchResults.find(b => b.albumId == albumId);
    if (book) {
        const shareText = `【Ajeo提示】请前往浏览器粘贴【链接】收听 \n  ${book.title} \n 【链接】：\n ${window.location.href.split('#')[0]}#albumId=${albumId}`;
...
function saveCurrentChapterIndex() {
    if (!state.currentBook) return;
    localStorage.setItem(`bookChapter_${state.currentBook.albumId}`, state.currentChapterIndex.toString());
}

function getSavedChapterIndex(albumId) {
    const saved = localStorage.getItem(`bookChapter_${albumId}`);
...
function initApp() {
    ...
    if (hash.startsWith('#albumId=')) {
        const albumId = hash.split('=')[1];
        loadBookDetails(albumId);
    } else if (hash.startsWith('#book_id=')) {
        // 兼容旧链接
