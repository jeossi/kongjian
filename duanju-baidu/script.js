...
// 代理服务器配置 - 只使用指定的代理服务器
const VIDEO_PROXY_URL = 'https://ajeo.cc/'; // 指定的代理服务器
...
let episodeList = [];
let hls = null; // HLS实例
let currentQuality = '1080p'; // 当前线路质量，默认蓝光
...
function searchDramas() {
    ...
    // 构建请求URL
    let requestUrl = `${API_BASE}?key=${API_KEY}&name=${encodeURIComponent(keyword)}&page=${currentPage}`;
    ...
    fetch(requestUrl, {
        ...
        .then(data => {
            if (data.code === 200 && data.data) {
                ...
                // 对于百度短剧API，我们假设总共有10页数据用于分页演示
                // 在实际应用中，您可能需要根据API返回的数据来确定总页数
                const totalPages = 10;
    ...
    list.forEach(drama => {
        ...
        html += `
            <div class="drama-item">
                <img src="${drama.cover || 'https://via.placeholder.com/250x200?text=暂无封面'}" alt="${drama.title}" class="drama-cover">
                <div class="drama-info">
                    <div class="drama-title">${drama.title}</div>
                    <div class="drama-author">集数：${drama.totalChapterNum || '未知'}</div>
                    <div class="drama-desc">${drama.title || '暂无简介'}</div>
                    <div class="drama-actions">
                        <button class="action-btn play-btn" onclick="playDrama('${drama.id}')">播放</button>
                        <button class="action-btn favorite-btn" onclick="toggleFavorite('${drama.id}', '${drama.title}', '${drama.cover || ''}')">${isFavorite ? '已收藏' : '收藏'}</button>
...
function showDramaDetails(dramaId) {
    ...
    // 构建请求URL
    let requestUrl = `${API_BASE}?key=${API_KEY}&id=${dramaId}`;
    ...
    fetch(requestUrl, {
        ...
        .then(data => {
            if (data.code === 200 && data.data) {
                ...
                // 设置短剧信息
                dramaTitle.textContent = data.title || '未知短剧';
                dramaDescription.textContent = `共${data.total || 0}集`;
                ...
                if (episodeList.length > 0) {
                    // 过滤出有效的播放链接
                    const validEpisodes = episodeList.filter(ep => ep.video_id);
                    if (validEpisodes.length > 0) {
                        ...
                        if (lastPlayedEpisode[dramaId]) {
                            const savedEpisodeIndex = validEpisodes.findIndex(ep => 
                                ep.video_id == lastPlayedEpisode[dramaId].video_id);
                        ...
                        playEpisodeByVideoId(selectedEpisode.video_id, episodeToPlay, selectedEpisode.title);
                    }
                }
...
function renderEpisodeList(list) {
    ...
    // 过滤出有效的剧集（有video_id）
    const validEpisodes = list.filter(ep => ep.video_id);
    ...
    let html = `
        ...
        <div class="episode-header">
            ...
            <div class="quality-selector">
                <button class="quality-btn uhd-btn ${currentQuality === '1080p' ? 'active' : ''}" onclick="switchQuality('1080p')">蓝光</button>
                <button class="quality-btn hd-btn ${currentQuality === 'sc' ? 'active' : ''}" onclick="switchQuality('sc')">超清</button>
                <button class="quality-btn sd-btn ${currentQuality === 'sd' ? 'active' : ''}" onclick="switchQuality('sd')">标清</button>
            </div>
        </div>
    ...
    list.forEach((episode, index) => {
        // 检查剧集是否有效（有video_id）
        const isValid = episode.video_id;
...
function playEpisodeByIndex(index) {
    if (episodeList && episodeList.length > index) {
        ...
        // 检查剧集是否有效
        if (episode && episode.video_id) {
            playEpisodeByVideoId(episode.video_id, index, episode.title);
...
// 通过video_id获取播放链接并播放
function playEpisodeByVideoId(videoId, index, title) {
    if (!videoId) {
        alert('播放地址无效');
        return;
    }
    ...
    // 显示正在播放的集数
    showEpisodeNumber(title || `第${index + 1}集`);
    ...
    // 构建请求URL
    let requestUrl = `${API_BASE}?key=${API_KEY}&video_id=${videoId}`;
    ...
    fetch(requestUrl, {
        ...
        .then(data => {
            ...
            if (data.data && data.data.statistics && statisticsContainer) {
                const stats = data.data.statistics;
                statisticsContainer.innerHTML = `
                    <span>播放: ${stats.views || 0}</span> | 
                    <span>点赞: ${stats.likes || 0}</span> | 
                    <span>评论: ${stats.comments || 0}</span> | 
                    <span>分享: ${stats.shares || 0}</span>
                `;
            } else {
                statisticsContainer.innerHTML = '';
            }
            ...
            // 检查是否有播放链接数据
            if (data.data && data.data.qualities && data.data.qualities.length > 0) {
                // 获取所有线路
                const qualities = data.data.qualities;
                
                // 按质量排序：蓝光(1080p) > 超清(sc) > 标清(sd)
                qualities.sort((a, b) => {
                    if (a.quality === '1080p') return -1;
                    if (b.quality === '1080p') return 1;
                    if (a.quality === 'sc') return -1;
                    if (b.quality === 'sc') return 1;
                    return 0;
                });
                ...
                // 首先尝试选择当前设置的质量
                const currentQualityObj = qualities.find(q => q.quality === currentQuality);
                if (currentQualityObj) {
                    playUrl = currentQualityObj.download_url || currentQualityObj.play_url;
                    qualityTitle = currentQualityObj.title || currentQualityObj.quality;
                }
                
                // 如果没有找到当前设置的质量，则选择最好的质量
                if (!playUrl && qualities.length > 0) {
                    playUrl = qualities[0].download_url || qualities[0].play_url;
                    qualityTitle = qualities[0].title || qualities[0].quality;
                }
                ...
                if (!playUrl) {
                    ...
                    for (let quality of qualities) {
                        if (quality.download_url || quality.play_url) {
                            playUrl = quality.download_url || quality.play_url;
                            qualityTitle = quality.title || quality.quality;
                            break;
                        }
                    }
                }
                ...
                console.log(`使用${qualityTitle}线路播放: ${playUrl}`);
                
                // 处理视频播放链接
                handleVideoPlayback(playUrl, playAttempted, playStartTime, index);
            } else if (data.data && (data.data.download_url || data.data.play_url)) {
                // 如果是直接返回播放链接的格式
                let playUrl = data.data.download_url || data.data.play_url;
                let qualityTitle = data.data.title || '默认线路';
                ...
                console.error('无法解析播放链接数据:', data);
                alert('无法获取播放链接，数据格式不正确');
            }
        })
        .catch(error => {
...
// 处理视频播放的通用函数
function handleVideoPlayback(playUrl, playAttempted, playStartTime, index) {
    // 使用指定的代理服务器来绕过防盗链
    console.log('使用指定代理服务器播放视频');
    
    // 使用指定的代理服务器
    const finalUrl = `${VIDEO_PROXY_URL}${playUrl}`;
    ...
    videoPlayer.play().then(() => {
        ...
        videoPlayer.play().then(() => {
            ...
            console.error('添加crossorigin后仍然播放失败:', error);
            // 如果是防盗链问题，提示用户启用代理
            if (error.message.includes('NotAllowedError') || error.name === 'NotAllowedError') {
                showPlaybackStatus('播放被拒绝，建议启用视频代理');
            } else {
    ...
    // 保存最后播放的集数
    if (currentDramaId && playUrl) {
        lastPlayedEpisode[currentDramaId] = { url: playUrl, index: index };
