...
const dom = {
    ...
    playerBookTotal: document.getElementById('player-book-total'), // 修改为显示总集数
...
async function performSearch() {
    ...
    try {
        const res = await fetch(`https://api.cenguigui.cn/api/duanju/hema.php?name=${encodeURIComponent(query)}&page=1`);
        const data = await res.json();
        if (data.code === 200 && data.data) {
            state.searchResults = data.data.map(book => {
                // 处理类型数组，将name值用"|"连接
                const typeStr = book.type.map(t => t.name).join(' | ');
                return {
                    ...book,
                    typeStr: typeStr,
                    play_cnt: book.followCount,
                    episode_cnt: book.totalChapterNum
                };
            });
    ...
    books.forEach(b => {
        ...
        card.innerHTML = `
            ...
            <div class="book-info">
                ...
                <div class="book-type">分类: ${b.typeStr}</div>
...
async function loadBookDetails(bookId) {
    try {
        const res = await fetch(`https://api.cenguigui.cn/api/duanju/hema.php?book_id=${bookId}`);
        const data = await res.json();
        if (data.code === 200 && data.data) {
            // 从搜索结果中查找书籍信息以获取作者和分类
            const searchBook = state.searchResults.find(b => b.book_id === bookId);
            
            state.currentBook = {
                book_id: bookId,
                book_name: data.book_name,
                author: searchBook ? searchBook.author : "未知演员",
                category: searchBook ? searchBook.typeStr : "未知分类",
                book_pic: data.book_pic,
                total: data.total // 总集数
            };
...
function renderPlayerPage() {
    ...
    const { book_name, author, category, book_pic, total } = state.currentBook;
    ...
    dom.playerBookTotal.textContent = `总集: ${total}集`; // 修改为显示总集数
...
async function playChapterAudio(chapter) {
    ...
    try {
        ...
        const res = await fetch(`https://api.cenguigui.cn/api/duanju/hema.php?book_id=${state.currentBook.book_id}&video_id=${chapter.video_id}`);
        const videoData = await res.json();
        if (videoData.code === 200 && videoData.data && videoData.data.url) {
            // 使用代理获取视频URL
            const videoUrl = proxyUrl(videoData.data.url);
    ...
    favorites.forEach(book => {
        ...
        item.innerHTML = `
            ...
            <div class="favorite-info">
                ...
                <div class="favorite-category">${book.typeStr || '未知分类'}</div>
            </div>
