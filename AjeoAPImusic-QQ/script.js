...
let currentLyrics = "";
let currentRawLyrics = ""; // 存储原始歌词（含时间戳）
let loopMode = false;
let isMuted = false;
let lastVolume = 1;
let baseApiUrl = "";
let currentBlobUrl = null; // 存储当前的Blob URL
let currentLyricsElements = []; // 存储歌词元素
let currentLyricsTimes = []; // 存储歌词时间点
...
function searchMusic(keyword) {
  baseApiUrl = `https://www.hhlqilongzhu.cn/api/dg_QQmusicflac.php?msg=${encodeURIComponent(keyword)}&type=json&n=`;
  ...
  fetch(baseApiUrl)
    ...
    .then(data => {
      ...
      results = results.map(item => ({
        ...
        songid: item.songid || item.id || item.n // 使用n作为后备ID
      }));
...
function playSong(song) {
  ...
  fetch(detailUrl)
    ...
    .then(data => {
      ...
      // 统一处理字段名称
      songDetail = {
        url: songDetail.music_url || songDetail.url || songDetail.music,
        lyric: songDetail.lyric || songDetail.lrc,
...
async function updatePlayer(songDetail) {
  ...
  if (songDetail.lyric) {
    // 存储原始歌词（含时间戳）
    currentRawLyrics = songDetail.lyric;
    
    // 处理歌词显示 - 移除时间戳并保留换行
    const cleanedLyrics = removeTimestamps(currentRawLyrics);
    lyricsContent.innerHTML = cleanedLyrics;
    
    // 重置歌词元素和时间点
    currentLyricsElements = Array.from(lyricsContent.children);
    currentLyricsTimes = [];
    
    // 解析歌词时间点
    parseLyricsTimestamps();
  } else {
    lyricsContent.textContent = "暂无歌词";
    currentRawLyrics = "";
    currentLyricsElements = [];
    currentLyricsTimes = [];
  }
  ...
  downloadLyricsBtn.disabled = !currentRawLyrics;
...
// 解析歌词时间点
function parseLyricsTimestamps() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (timeMatch) {
      ...
      // 存储时间点和对应的歌词行索引
      if (lineIndex < currentLyricsElements.length) {
        currentLyricsTimes.push({
          time: lineTime,
          index: lineIndex
        });
        
        // 为歌词行设置时间属性
        currentLyricsElements[lineIndex].setAttribute('data-time', lineTime);
        lineIndex++;
      }
    }
  }
...
function removeTimestamps(lyrics) {
  // 移除时间戳并保留换行符
  return lyrics
    .replace(/\[\d{1,2}:\d{1,2}(?:\.\d{1,3})?\]/g, '') // 移除时间戳
    .replace(/\\n/g, '\n') // 处理转义的换行符
    .replace(/\n/g, '<br>'); // 替换为HTML换行标签
}
...
function downloadLyrics(title, artist) {
  ...
  const blob = new Blob([currentRawLyrics], { type: 'text/plain' });
...
function syncLyrics(currentTime) {
  if (currentLyricsTimes.length === 0 || currentLyricsElements.length === 0) return;
  
  // 找到当前时间对应的歌词行
  let activeLine = null;
  
  for (let i = 0; i < currentLyricsTimes.length; i++) {
    if (currentLyricsTimes[i].time <= currentTime) {
      activeLine = currentLyricsTimes[i];
    } else {
      break;
    }
  }
  ...
  // 移除所有高亮
  currentLyricsElements.forEach(el => {
    el.classList.remove('current-lyric');
  });
  
  // 高亮当前行
  const currentElement = currentLyricsElements[activeLine.index];
  currentElement.classList.add('current-lyric');
  
  // 滚动到当前歌词行（仅在展开时）
  if (lyricsExpanded) {
    const containerHeight = lyricsContainer.clientHeight;
    const elementHeight = currentElement.offsetHeight;
    const scrollTop = currentElement.offsetTop - (containerHeight / 2) + (elementHeight / 2);
    lyricsContent.scrollTop = scrollTop;
  }
}
