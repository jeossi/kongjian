// 修复版 script.js（播放结束后立即更新锁屏信息，其余逻辑保持）
const hotKeywords = [
...
function renderHotTags() {
  ...
  closeBtn.addEventListener('click', () => { hotSearchPanel.style.display = 'none'; });
...
/* ---------- 播放控制 ---------- */
function playSong(song, skipRefreshLockScreen = false) {
  // 如果 skipRefreshLockScreen=true，说明后面会统一刷新，跳过第一次刷新
  if (!skipRefreshLockScreen) {
    resetMediaSession();
    audioPlayer.pause();
  }
...
function highlightCurrentSong(song) {
  ...
  if (item) { item.classList.add('current-song'); item.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
}
...
function playNextSong() {
  ...
  /* 1. 立即更新锁屏与 UI（不等待音频） */
  songTitle.textContent = next.title;
  songArtist.textContent = next.singer;
  albumCover.innerHTML = '<i class="fas fa-music"></i>'; // 占位
  updateMediaSessionMetadata(next); // 只传 title/artist
  lyricsContent.textContent = '加载中...';

  /* 2. 再异步拉音频 */
  fetch(`${baseApiUrl}${next.n}`)
    .then(r => r.json())
    .then(data => data && data.data && updatePlayer(data.data))
    .catch(() => { lyricsContent.textContent = '加载歌曲详情失败'; });
}
...
function updateMediaSessionMetadata(song, coverUrl) {
  ...
  const artwork = [];
  if (coverUrl) {
    artwork.push({ src: getSecureImageUrl(coverUrl), sizes: '500x500', type: 'image/jpeg' });
  }
...
function updatePlayerProgress() {
  ...
  if ('mediaSession' in navigator && !isNaN(duration)) {
    navigator.mediaSession.setPositionState({ duration, playbackRate: audioPlayer.playbackRate, position: currentTime });
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  resultsList.addEventListener('click', e => {
    ...
    if (btn) {
      ...
      const song = currentSearchResults.find(s => s.n === n);
      if (!song) return;
      isFavorited(song, currentSearchKeyword) ? removeFavorite(song, currentSearchKeyword) : addFavorite(song, currentSearchKeyword);
      return;
    }
    ...
    if (shareBtn) {
      ...
      navigator.clipboard.writeText(shareText).then(() => alert('已复制请到微信粘贴分享')).catch(() => {
        const ta = document.createElement('textarea'); ta.value = shareText; document.body.appendChild(ta);
        ta.select(); document.execCommand('copy'); document.body.removeChild(ta); alert('已复制请到微信粘贴分享');
      });
      return;
    }
  ...
  audioPlayer.addEventListener('play', () => { playBtn.style.display = 'none'; pauseBtn.style.display = 'flex'; if ('mediaSession' in navigator) navigator.mediaSession.playbackState = "playing"; });
  audioPlayer.addEventListener('pause', () => { playBtn.style.display = 'flex'; pauseBtn.style.display = 'none'; if ('mediaSession' in navigator) navigator.mediaSession.playbackState = "paused"; });
