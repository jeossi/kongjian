...
function searchMusic(keyword, callback = null) {
  ...
  baseApiUrl = `https://www.hhlqilongzhu.cn/api/dg_mgmusic.php?gm=${encodeURIComponent(keyword)}&n=`;
  ...
  fetch(baseApiUrl)
    ...
    .then(data => {
      // 处理新的API响应格式
      const results = data && data.code === 200 && data.data ? data.data : [];
      renderSearchResults(results);
      // 只有在不是处理分享链接时才自动播放第一首
      if (!isHandlingShareLink && results && results.length) playSong(results[0]);
...
async function updatePlayer(detail) {
  ...
  // 处理音频URL - 使用新API的music_url字段
  let audioUrl = detail.music_url;
  if (detail.music_url && detail.music_url.includes('douyinvod.com')) {
    try {
      const resp = await fetch(PROXY_SERVER + encodeURIComponent(detail.music_url));
  ...
  // 更新歌曲信息
  currentSong.url = detail.music_url;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderHotTags();
  if (!handleShareUrl()) searchMusic('王佳音');
  renderFavorites();
